					...
					<div id="slidercon">
						...
						<div id="about" style="overflow: scroll; height: 390px;">
							...
							<ul class="price-list p2">
								...
								for ($i = 0; $row = $result->fetch(); $i++) {
									...
									<li><span>Rm <?php $dsds = $row['price'];
											...
											?></span><a rel="facebox"
									                    href="menudetails.php?id=<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a>
									</li>
									<?php
								}
								?>
							...
							<ul class="price-list p2">
								...
								for ($i = 0; $row = $result->fetch(); $i++) {
									?>
									<li><span><?php echo $row['price']; ?></span><a rel="facebox"
									                                                href="menudetails.php?id=<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a>
									</li>
									<?php
								}
							...
							<ul class="price-list p2">
								...
								<br><strong style="font-size: 12px; font-style:italic;">( Combo 285 - 2 pasta/2 meat/3
									bread/2 dessert/softdrinks )</strong>
								<?php
								$id = 'merienda';
								$result = $db->prepare("SELECT * FROM menu WHERE mcat= :mmm");
								$result->bindParam(':mmm', $id);
