...
if ((($mcat == 'sarapan') AND ($nopax > 99)) OR (($mcat == 'minum petang') AND ($nopax > 49)) OR (($mcat == 'makan tengahari/malam') AND ($nopax > 29))) {
	...
	$q = $db->prepare($sql);
	$q->execute([':a' => $resid, ':b' => $name, ':c' => $address, ':d' => $motif, ':e' => $toc, ':f' => $tre, ':g' => $d, ':h' => $nopax, ':i' => $cnum, ':j' => $vn, ':k' => $mcat, ':l' => $lname, ':m' => $stat, ':n' => $time, ':z' => $resdate]);
